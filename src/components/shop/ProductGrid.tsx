...
export const ProductGrid = ({ products, categoryName, isEmpty, isLoggedIn, isApproved }: ProductGridProps) => {
        ...
        <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
                {products.map((product) => (
                    <ProductCard key={product.id} product={product} isLoggedIn={isLoggedIn} isApproved={isApproved} />
                ))}
            </div>
        </div>
    );
